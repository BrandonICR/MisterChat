...
public class MessageHandler {
    ...
    public void manage(Message messageReceived){
        ...
        if(messageReceived.getConfig().isNewUser() && directoryHandler.existsContact(messageReceived.getSender())){
            ...
            String message = Utils.formatDate(messageReceived.getBuildingDate())+":: ¡Se ha unido "+messageReceived.getSender().getUserName()+" al chat!";
            ...
            String message = Utils.formatDate(messageReceived.getBuildingDate())+"::"+(String)ObjectValidator.ifTrueReturnOrElse(messageReceived.getConfig().isPrivate(), "PRIVATE::", SpecialCharacterConstants.STR_EMPTY)+messageReceived.getSender().getUserName()+"::"+messageReceived.getText();
            System.out.println(message);
            showMessage(message);
        }
    }
